...
def test_diff_cli_exits_zero_when_no_regression(cli_runner: CliRunner, tmp_path: Path) -> None:
    cell = "platform=ios,python=3.12"
    baseline = _write_json(tmp_path / "baseline.json", _baseline_payload({cell: 8}))
    candidate = _write_json(tmp_path / "candidate.json", _baseline_payload({cell: 8}))
...
def test_diff_cli_exits_one_and_lists_cell_when_regression(
    ...
    cell = "platform=ios,python=3.12"
    baseline = _write_json(tmp_path / "baseline.json", _baseline_payload({cell: 8}))
    candidate = _write_json(tmp_path / "candidate.json", _baseline_payload({cell: 5}))
...
def test_diff_cli_tolerance_absorbs_small_regression(cli_runner: CliRunner, tmp_path: Path) -> None:
    cell = "platform=ios,python=3.12"
    baseline = _write_json(tmp_path / "baseline.json", _baseline_payload({cell: 8}))
    candidate = _write_json(tmp_path / "candidate.json", _baseline_payload({cell: 7}))
